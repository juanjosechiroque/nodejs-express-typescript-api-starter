...
export async function registerUser({ email, password }: UserCredentials) {
    ...
    const existingUser = await findUserByEmail({ email: normalizedEmail });
    ...
    let user;
    try {
        user = await createUser({ email: normalizedEmail, password });
    } catch (err) {
...
export async function loginUser({ email, password }: UserCredentials) {
    const normalizedEmail = email.trim().toLowerCase();
    const user = await findUserByEmail({ email: normalizedEmail });
