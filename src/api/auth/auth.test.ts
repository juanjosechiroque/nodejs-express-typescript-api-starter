...
describe("API: POST /v1/auth/signup", () => {
    describe("Given the API is available", () => {
        it("When the request is sent, then returns a JWT when registration succeeds", async () => {
            const userMocked = {
                _id: "1",
        ...
        it("When the request is sent, then returns 400 on duplicate key race condition", async () => {
            mockFindOne(null);
        ...
        it("When the request is sent, then returns 400 when the request body is missing", async () => {
            const response = await api.post(`${V1}/auth/signup`);
            expect(response.status).toBe(400);
        });

        it("When the request is sent, then returns 400 when password is shorter than 8 characters", async () => {
            const response = await api
                .post(`${V1}/auth/signup`)
...
describe("API: POST /v1/auth/login", () => {
    describe("Given the API is available", () => {
        it("When the request is sent, then returns a JWT when credentials are valid", async () => {
        ...
        it("When the request is sent, then returns 401 when the user does not exist", async () => {
            mockFindOne(null);
        ...
        it("When the request is sent, then returns 401 when the password is wrong", async () => {
